...
if logPerformanceAtBestIter:
  saveFilename=tmpSavePrefix+".test.aucBI"
  np.save(saveFilename, reportAUCBestIter)
  saveFilename=tmpSavePrefix+".test.apBI"
  np.save(saveFilename, reportAPBestIter)

if savePredictionsAtBestIter:
  if useDenseOutputNetPred:
    saveFilename=tmpSavePrefix+".test.predBI.pckl"
    saveFile=open(saveFilename, "wb")
    pickle.dump(predDenseBestIter.astype(np.float32), saveFile)
    saveFile.close()
  else:
    saveFilename=tmpSavePrefix+".test.predBI.pckl"
    saveFile=open(saveFilename, "wb")
