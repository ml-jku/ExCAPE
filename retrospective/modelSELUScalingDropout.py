...
if basicArchitecture[1]=="seluScaling":
...
with tf.variable_scope('layer_'+str(0)):
  ...
  if nrDenseFeatures>0.5:
    ...
    hiddenActModl.append(xDenseData*tf.to_float(tf.random_uniform([sh0, nrDenseFeatures])<(1.0-inputDropout)))
  if nrSparseFeatures>0.5:
    ...
    if not (normalizeGlobalSparse or normalizeLocalSparse):
      ...
      hiddenActModl.append(xDenseDataFromSparse*tf.to_float(tf.random_uniform([sh0, nrSparseFeatures])<(1.0-inputDropout)))
...
with tf.variable_scope('layer_'+str(layernr)):
  ...
  if nrSparseFeatures>0.5:
    ...
    if not (normalizeGlobalSparse or normalizeLocalSparse):
      ...
      #idTensors.append(sparseMeanWSparse)
