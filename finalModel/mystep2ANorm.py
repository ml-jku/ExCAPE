...
#python3 $HOME/mysources/pipeline2/excapedb/mystep2ANorm.py -maxProc 10 -availableGPUs -sizeFact 1.0 -dataset ecfp6 -ofolds 0 1 2 -pStart 0 -pEnd 32 -continueComputations -saveComputations -startMark start -finMark finished -epochs 300 -mode simple
...
parser.add_argument("-availableGPUs", help="Available GPUs", nargs='*', type=int, default=[4])
parser.add_argument("-sizeFact", help="Size Factor GPU Scheduling", type=float, default=1.0)
parser.add_argument("-originalData", help="Path for original data", type=str, default="/mydatasets/excapedb/current/")
...
parser.add_argument("-ofolds", help="Outer Folds", nargs='+', type=int, default=[-1])
...
for outerFold in compOuterFolds:
  ...
  for paramNr in mycompParams:
    ...
    saveFile.close()
    dbgOutput=open(dbgPath+savePrefix0+".dbgA", "w")
    ...
    saveScript=basePath+'step2Save.py'
    runEpochs=True
